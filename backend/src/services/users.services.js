...
const getRelativePath = (absolutePath) => {
  ...
  return `uploads/profile_pics/${path.basename(normalized)}`;
};
...
const loginUser = async (email, password) => {
  try {
    ...
    return { success: true, message: "Login successful", data: { user: userObject, token } };
  } catch (error) {
...
const deactivateUser = async (userId) => {
  try {
    // Permanently delete user from MongoDB
...
const updateAvatar = async (userId, file) => {
  try {
    ...
    const avatarUrl = getRelativePath(file.path);
    user.avatar = avatarUrl;
    await user.save();
    return { success: true, message: "Avatar updated successfully", data: { user, avatarUrl } };
  } catch (error) {
...
const deleteAvatar = async (userId) => {
  try {
    ...
    user.avatar = null;
    await user.save();
    return { success: true, message: "Avatar removed successfully", data: { user } };
  } catch (error) {
...
module.exports = {
  ...
  getAllUsers, changePassword, deactivateUser, updateAvatar, deleteAvatar,
};